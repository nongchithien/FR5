...
robot_recv_thread::robot_recv_thread(const std::string node_name):rclcpp::Node(node_name){
    ...
    if(_socketfd1 == -1){
        RCLCPP_ERROR(rclcpp::get_logger(LOGGER_NAME),"%s errno=%d (%s)",
            msgout[msg_id(socket_create_failed)], errno, strerror(errno));
        return; // Socket creation failed, exit this constructor safely.
    }else{
        ...
        if(0 != res1){
            RCLCPP_ERROR(rclcpp::get_logger(LOGGER_NAME),"%s errno=%d (%s)",
                msgout[msg_id(socket_connect_failed)], errno, strerror(errno));
            close(_socketfd1);
            _socketfd1 = -1;
            return; // Connection failed, exit this constructor safely.
        }else{
